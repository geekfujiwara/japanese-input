...
- モデルの使い分け: 開発はClaude Opus 5.5で行う。SLMは辞書作成環境（BYOKでローカルのOllama / Foundry Local / llama.cppにつなぐ）と、IME本体の文脈変換・もしかして（計画書4.5.2。変換サーバー内のllama.cppで端末内だけで動かし、モデルはパッケージに同梱する）で使う。IME本体と辞書作成環境からクラウドのモデルを呼ばない
