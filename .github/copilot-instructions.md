...
- モデルの使い分け: 開発はClaude Opus 5.5で行う。SLMは辞書作成環境の中でだけ使い、BYOKでローカル（Ollama / Foundry Local / llama.cpp）につなぐ。SLMに任せられる作業は任せて、開発のトークン数を減らす。IME本体ではSLMを使わず、もしかしてや文脈の判断はアルゴリズムで行う（2026-09-26の判断、[docs/slm-evaluation-2026-09.md](../docs/slm-evaluation-2026-09.md)）。IME本体と辞書作成環境からクラウドのモデルを呼ばない
...
- CIの結果を確認し、失敗したら同じブランチで直す。CIがすべて成功したら、PRを自動でマージする（`gh pr merge --merge --delete-branch`、ユーザーの指示 2026-09-24）
