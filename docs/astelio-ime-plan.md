...
| R-07 | OS起動時に自動起動できる |
| R-08 | 初回起動時にセットアップウィザードを表示する。インストーラーから同じウィザードを開き、好みの設定（切替キー、句読点・記号、もしかして、学習など）を選んで導入できる |
...
| ID | 機能 |
| --- | --- |
| B-01 | ローマ字→ひらがな変換（未確定表示、下線、カーソル移動、BackSpace/Delete）。Google日本語入力と同じzで始まる記号（zh ←、zl →、z. … など） |
| B-02 | Spaceでかな漢字変換。文節区切り、文節の伸縮（Shift+←/→）、文節間の移動。入力ミス（「っっ」「んん」など）を補正した候補も出す |
| B-03 | 候補ウィンドウ。候補番号の選択、ページ送り、注釈（用法・同音異義語の説明） |
| B-04 | 予測変換（入力途中で候補を表示）。Tabで選択 |
| B-05 | F6〜F10でひらがな・カタカナ・半角カナ・全角英数・半角英数に変換 |
| B-06 | Enterで確定、Escで取り消し、一部確定（Ctrl+↓で注目文節までを確定し、残りは変換中のまま） |
| B-07 | 再変換（確定済み文字列の選択から変換し直す） |
| B-08 | 確定取り消し（確定の直後にCtrl+BackSpaceで、確定した文字列を変換中に戻す。選んだ候補と区切りも戻す） |
| B-09 | 数字変換（漢数字、全角、桁区切り）、日付・時刻変換（「きょう」「いま」など） |
| B-10 | 記号・絵文字・顔文字の変換（「やじるし」→「→」など） |
| B-11 | パスワード入力欄では無効化し、キーをそのままアプリに渡す（WindowsはInputScopeのパスワード・PIN、macOSはSecure Input） |
| B-12 | 入力モード表示（あ / A）。Alt単押しやタスクバーでの切替時に、入力位置の近くへ約1.2秒表示 |
| B-13 | 絵文字パレット。「えもじ」と入力するとパレットを表示し、Tab/↓で開く（Spaceでは通常どおり「絵文字」に変換する）。パレット内の読みによる検索、分類（履歴・顔・人・自然・食べ物・活動・旅行・物・記号・旗）ごとの一覧、使った絵文字の履歴（新しい順、最大32個、アプリ間で共有）。マウスでも選べる |
| B-14 | もしかして。打ち間違えた読み（「ゆーあー」→「ユーザー」など）を辞書とアルゴリズムで直し、前の文脈も使って選び、通常の候補と同じウインドウに「もしかして:」として表示する。SLMは使わない（4.5.2） |
...
- 英字・数字・スペースは半角。Shift+英字で一時英数になり、確定・取り消し・Shiftの単押しで戻る
- 記号は半角。ただし `[` `]` `/` `-` `!` `?` は和文記号（「 」 ・ ー ！ ？）、`,` `.` は句読点のスタイル（既定は 、。）
- 修飾キーの単押しは、押してから離すまで1秒以内のものだけとする
...
| D-04 | 自動学習（選んだ候補の順位、文節区切り、語の組み合わせ、予測候補）。選んだ候補は次から候補ウインドウの先頭に出し、履歴の印を付ける。語の組み合わせは「直前の語と読み」ごとに選んだ候補を覚え、読みだけの学習より優先する（お→箸、川の→橋）。文節区切りはShift+←/→で直して確定した区切りを読みごとに覚える |
| D-05 | 学習履歴の閲覧、個別削除、期間を指定した削除（過去1時間 / 24時間 / 7日間 / 4週間に使ったもの）、全削除。候補ウインドウでは Ctrl+Delete で選んだ候補を履歴から消す。タスクバーの入力モードのアイコンを右クリックして、履歴の使用の切替、管理画面（一覧・削除・期間を指定した削除）、全削除ができる |
| D-06 | 学習の一時停止（シークレットモード: 履歴は使うが記録しない）。アプリ単位でも設定できる。Windowsはタスクバーの右クリックメニューで切り替え、HKCU\Software\AstelioIME の `LearningPaused`、`NoLearningApps`（exeのファイル名）に保存する |
...
- 辞書: 読みで整列した配列を二分探索する（前方一致は1文字ずつ範囲を絞る）。ファイルはそのままメモリに置いて使える形式で、読み込み時にすべての位置と範囲を検証する。速度が足りなければLOUDSやダブル配列に切り替える
...
| azooKey_dictionary_storage | システム辞書、連接コスト | Apache-2.0 | azooKeyのバイナリ形式（LOUDSのノードごとのエントリー、品詞の連接コスト行列）。辞書ツールでAstelioの6列TSV（読み・表記・左品詞ID・右品詞ID・意味ID・コスト）に変換する。版はコミットとSHA-256で固定する。LICENSEの同梱が必要 |
...
### 4.5.2 もしかしてと文脈の判断（アルゴリズム）

IME本体ではSLMを使わず、辞書とアルゴリズムだけで実装する（利用者の判断 2026-09-26。根拠は [SLMの評価結果](slm-evaluation-2026-09.md)）。SLMをパッケージに含めるオプションも用意しない。SLMは開発時の辞書作成（4.5.1）でだけ使う。

- もしかして（B-14）: 辞書が、打ったキーからキー1つの抜け・余分・隣のキー・入れ替わりを直した読みの語を候補にし（`FindTypoCandidates`）、語のコストと前の文脈で選ぶ。よくある打ち間違いの規則（f/h などローマ字のゆれ、活用の断片の除外）を打ち間違いのコーパスで確かめながら増やす
- 文脈の判断: 前の文脈（直前に確定した語）との連接コスト・共起、入力の履歴（D-04）で候補の順位を補正する。評価コーパスで正解率が下がらないものだけ採用する。直前の語は、辞書の第1候補を確定したときだけ使い、カーソルを動かすキー・フォーカスの移動・かなのままの確定で忘れる（T-B02-5）
- もしかしての表示: 文節の自立語の部分を直した語を、助詞を付けて候補の2番目に入れ、候補ウインドウをすぐに開く。打ち間違いで辞書が細かく分けた入力（`ゆーあー`）は、全体を1つの文節にする。24キーより長い語は探さない
- もしかしては読みの異なる候補なので、通常の候補とは分けて「もしかして:」の行に出す
- パスワード欄やシークレットモードでは履歴を使わず、履歴を保存しない

**負荷の目標**

| 項目 | 目標 |
| --- | --- |
| もしかしての候補生成 | 1語あたり p95 2ms以下（実測 1.7ms） |
| 常駐メモリの増加 | 辞書以外に数MB以内（履歴を含む） |
...
| sarashina2.2（0.5B / 1B） | SB Intuitions | 0.5B / 1B | MIT（2026-09-26にLICENSEを確認。商用利用・再配布可、利用に同意が必要な制限なし） | 同上の小型版 | 辞書作成での軽い処理の候補（IME本体では使わない） |
...
| zenz-v2.5 | かな漢字変換専用で有力だが、CC BY-SA 4.0 |
| Qwen2.5-3B-Instruct | Qwen独自のライセンス（同じシリーズでも0.5B・1.5BはApache-2.0） |
| gemma-2-2b-jpn-it | Gemma独自の利用規約 |

**IME本体での利用（検討の記録）**

商用利用・再配布が可能な2B以下の8モデル（sarashina2.2 0.5B/1B、Qwen2.5 0.5B/1.5B、Qwen3 0.6B/1.7B、llm-jp-3-1.8b、RakutenAI-2.0-mini）をもしかしてで比較したが、アルゴリズムを上回らなかったため、IME本体ではSLMを使わない（[SLMの評価結果](slm-evaluation-2026-09.md)）。比較の仕組み（`slm/`、`.github/workflows/slm-eval.yml`）は手動で再実行できるように残す。
...
- IME本体: SLMは使わない（4.5.2）。開発では、辞書作成などSLMに任せられる作業を任せて、開発用の大きなモデルのトークン数を減らす
...
| 学習履歴 | テキスト（UTF-8、タブ区切りで種類・直前の語・読み・表記・回数・順番・最後に使った時刻。最大5000件、古いものから消す。最初の形式も読める） | `%LOCALAPPDATA%\AstelioIME\learning.tsv` | 同上フォルダ |
| システム辞書 | バイナリ（読み取り専用） | インストール先 | アプリバンドル内 |

- 既存の `%LOCALAPPDATA%\AstelioIME\settings.json` は初回起動時に移行する
- 学習履歴は変更のたびにファイル全体を一時ファイル経由で書き直すので、削除した語はファイルに残らない。複数のアプリが同じファイルを使うため、入力の始めに更新時刻を見て読み直す（フェーズ4で変換サーバーに移す）
- パスワード欄・PIN欄（InputScope）では学習しない
...
- 背景はすりガラスのように背後をぼかす（Windows: ウィンドウ合成属性のアクリルぼかし（`SetWindowCompositionAttribute` の `ACCENT_ENABLE_ACRYLICBLURBEHIND`）と角丸 `DWMWA_WINDOW_CORNER_PREFERENCE`。候補ウィンドウはアクティブにならないため、非アクティブ時に単色になるDWMのシステム背景 `DWMWA_SYSTEMBACKDROP_TYPE` は代替としてだけ使う。macOS: `NSVisualEffectView`）
...
- 文節の区切り・伸縮・移動
- 候補ウィンドウ（Windows）
- 予測候補の基本（入力中に辞書の前方一致から表示し、Tab/↓で選ぶ。学習による予測はフェーズ5）
- 入力ミスの補正（重なった「っ」「ん」「ー」・小書きのかなを1つにした読みの語を、少し不利なコストで候補に入れる）
- F6〜F10の変換、数字・日付変換
- 評価コーパスと評価ツール（`eval/`。文正解率と上位5件の正解率を分類ごとにCIで記録し、0.5ポイント以上の悪化で失敗にする）
- 絵文字パレット（B-13。絵文字はシステム辞書に含まれるものを分類して並べる。履歴は `%LOCALAPPDATA%\AstelioIME\emoji_recent.txt`）
...
- 学習（候補順位、文節区切り、語の組み合わせ、予測）— 実装済み（TIP内で共有ファイルを使う。フェーズ4でサーバーに移す）
- 予測変換
- ユーザー辞書の追加・編集・インポート・エクスポート
- 学習履歴の閲覧・削除（個別、期間、すべて）— 実装済み。シークレットモード（履歴を使いつつ記録しない）
- 抑制語
...
- Windows: WiXインストーラー（TIP 3種の登録、サーバー、設定アプリ、辞書）、コード署名、アンインストール時の登録解除
- インストーラーの最後に設定アプリのウィザードを開き、次を選んで始められるようにする（R-08）。あとから設定アプリで変えられる
  - 日本語・英語の切替キー（左右Alt単押しなど）
  - 句読点と記号の入力（C-01〜C-03）
  - もしかしてのオン・オフ
  - 入力の履歴（学習）と予測の有効・無効
- macOS: Universal 2のパッケージ、署名、公証（同じウィザードをSwiftUIで用意する）
- 辞書の更新配布の仕組み
- 自動更新の確認機能
...
- 性能の計測と改善（3.5の目標値）
- 各種アプリでの互換性確認
- 文脈の判断ともしかしてのアルゴリズムの強化（4.5.2）。打ち間違いの文例を増やして数値で追う
...
8. ~~SLMによる文脈変換を提供するか~~ → 提供しない（2026-09-26に決定。[SLMの評価結果](slm-evaluation-2026-09.md)）
...
## 12. まだ実装していない機能と次の候補（2026-09-26時点）

実装済み: B-01〜B-02、B-04〜B-06、B-08〜B-14（B-07を除く）、D-01、D-04〜D-06（Windows TIP）。次の順で進める案。優先度は日常の使い勝手への影響で付けた。「済」は実装した項目と、残った課題。

| 優先 | ID | 内容 | 補足 |
| --- | --- | --- | --- |
| 済 | B-11 | パスワード欄・PIN欄で日本語入力にしない | InputScopeで判定し、キーをそのままアプリに渡す |
| 済 | B-08 | 確定取り消し（確定直後のCtrl+BackSpaceで変換中に戻す） | 確定した文字列がカーソルの直前にあるときだけ消す。残り: 学習の記録は取り消さない |
| 済 | B-06 | 一部確定（Ctrl+↓で注目文節までを確定し、残りを変換中のまま残す） | |
| 済 | B-12 | 入力モードの切替時に、カーソルの近くへ「あ / A」を一時的に表示する | 外部からのオン・オフでは表示しない。残り: 位置と時間を実機で確認する |
| 済 | D-06 | シークレットモード（履歴を使いながら記録しない）とアプリごとの設定 | 右クリックメニュー「記録を一時停止」「このアプリでは記録しない」。残り: ブラウザーのプライベートウィンドウの自動判定 |
| 次 | D-02・D-03・D-08 | ユーザー辞書（追加・編集・インポート・エクスポート）と抑制語 | 管理画面は入力履歴の画面と同じ作りにできる |
| 中 | B-07 | 再変換（確定済みの文字列を選んで変換し直す。TSFの `ITfFnReconversion`） | |
| 中 | B-03 | 候補の注釈（同音異義語の使い分け、「環境依存」など） | 辞書に注釈の欄を足す |
| 中 | — | 学習のアプリ間の即時共有 | 今は入力の始めにファイルを読み直す。フェーズ4の変換サーバーで解決する |
| 中 | — | もしかしての残りの外れ（打った読みも語になる例） | 入力の履歴と語の組み合わせで補う。組み合わせの学習を評価コーパスで測る |
| 低 | C-01〜C-10 | 設定アプリ（フェーズ6）。もしかしてのオン・オフ、履歴の使用などは今はレジストリと右クリックメニューで切り替える | |
| 低 | R-06〜R-08 | インストーラーとウィザード（フェーズ8） | 今は開発用のスクリプトで登録する |
| 低 | — | ARM64版Windowsのx64アプリ対応（ARM64Xの転送DLL） | |
| 低 | — | macOS版（フェーズ7） | |
