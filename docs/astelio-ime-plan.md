...
| R-07 | OS起動時に自動起動できる |
| R-08 | 初回起動時にセットアップウィザードを表示する。インストーラーから同じウィザードを開き、好みの設定（切替キー、句読点・記号、もしかしての方式と省電力、学習など）を選んで導入できる |
...
| B-14 | もしかして。打ち間違えた読み（「ゆーあー」→「ユーザー」など）を前後の文脈からSLMで推測し、通常の候補と同じウインドウに「もしかして:」として遅れて表示する。推測中は読み込み中のアニメーションを表示する（4.5.2） |
...
### 4.5.2 SLMによる文脈変換と「もしかして」

辞書で作った候補を、前後の文脈に合わせてSLMが選び直す（文脈変換）。変換そのものは辞書で行い、SLMは候補の順位付けだけを担う。さらに、読みの打ち間違いをSLMが推測して「もしかして」として示す（B-14）。

- モデルは本体のパッケージに同梱する（利用者の指示 2026-09-26）。同梱できるのは商用利用と再配布が許されたMIT・Apache-2.0のモデルだけで、ライセンス文を一緒に入れ、`THIRD_PARTY_NOTICES` に記録する。設定で無効にできる
- 推論は変換サーバー（フェーズ4）に組み込んだllama.cpp（GGUF）で行い、Windows ARMではNPU・GPU、macOSではMetalを使う。TIP（各アプリに読み込まれるDLL）にはモデルを読み込まない
- 文脈変換は時間内（目安 30ms）に終わらなければ、辞書だけの結果を表示する。入力した読みと合わない候補は採用しない（読みとの照合を必ず行う）
- もしかしては非同期で、結果が遅れて出てよい。読みの異なる候補なので、通常の候補とは分けて「もしかして:」の行に出し、推測中は読み込み中のアニメーションを出す。入力が進んだら古い推測は捨てる
- もしかしての方式: 辞書が、打ったキーからキー1つの抜け・余分・隣のキー・入れ替わりを直した読みの語を候補にする（`FindTypoCandidates`、数百マイクロ秒で終わる）。候補から選ぶのは、語のコストを比べるアルゴリズムかSLM。SLMに語を自由に書かせる方式は、比較テストで的中率が6.7%以下だったため採用しない（2026-09-26）。SLMが要るかどうかもT-S06で比べて決める
- SLMは候補があるときだけ、入力が止まってから呼ぶ。バッテリー駆動・省電力モードでは止めてアルゴリズムだけにし、一定時間使わないときはモデルをメモリから外す。最初に同梱するのは軽量なsarashina2.2-0.5b（Q4_K_M、約500MB）を第一候補とする（利用者の指示 2026-09-26。T-S06の結果で確定）

**想定する負荷（目標）**

| 項目 | 目標 | 考え方 |
| --- | --- | --- |
| 候補の生成 | 1語あたり 1ms以下 | 辞書引き数百回。常に動いても負荷は無視できる |
| SLMの呼び出し回数 | 入力した語の数%程度 | 候補がある語だけ。正しい読みの多くは候補がない（T-S06で呼び出し率を記録する） |
| SLM 1回のCPU時間 | 300ms以下（2スレッド） | 指示文と例はキャッシュし、出力は番号1文字だけ |
| 常駐メモリ（SLMあり） | 800MB以下 | モデル約500MB＋文脈の領域。使わないときは解放する |
| バッテリー | バッテリー駆動時はSLMを既定で止める | ウィザードと設定で変えられる |

**比較の結果（T-S06、2026-09-26、打ち間違い30件・正しい読み15件）**

| 方式 | 的中率 | 誤提案率 | 1回のCPU時間 | ピークメモリ |
| --- | ---: | ---: | ---: | ---: |
| algo-cost（SLMなし） | 66.7% | 0.0% | 候補の生成 p95 1.7ms | 辞書のみ |
| sarashina2.2-0.5b Q4_K_M | 60.0% | 20.0% | 630ms | 987MB |
| RakutenAI-2.0-mini Q8_0 | 70.0% | 26.7% | 2,092ms | 1,724MB |

- 辞書の候補に正解が入る割合は83.3%。SLMはアルゴリズムを明確には上回らず、誤提案が増え、CPU時間とメモリの負荷が大きい。このため、もしかしての既定は「軽量」（辞書とアルゴリズム）とし、SLMはまだ同梱しない
- アルゴリズムが外すのは、打った読みも語になる場合（しちょう→資料、ひかん→時間）と、活用の断片が候補に混じる場合（だんしゃ→出しゃ）。次は断片を候補から除き、打った読みが語になる場合だけSLMに聞く組み合わせと、コーパスの拡充を試す。SLMの第一候補は、最も軽いsarashina2.2-0.5b Q4_K_Mとする
...
| sarashina2.2（0.5B / 1B） | SB Intuitions | 0.5B / 1B | MIT（2026-09-26にLICENSEを確認。商用利用・再配布可、利用に同意が必要な制限なし） | 同上の小型版 | 文脈変換・もしかしての候補（下の表） |
...
**IMEに同梱するSLM（文脈変換・もしかして）の候補（2026-09-26にHugging FaceのモデルカードとLICENSEで確認）**

条件は、商用利用と再配布が許され、利用に同意が必要な制限や追加の規約がないこと。この条件で絞った候補を、T-S06の比較テストで選ぶ。

| モデル | 開発元 | 規模 | ライセンス | GGUF | 備考 |
| --- | --- | --- | --- | --- | --- |
| sarashina2.2-0.5b-instruct-v0.1 | SB Intuitions | 0.5B | MIT | なし（固定したllama.cppで変換する） | 日本語特化。開発元の評価では同規模のQwen2.5-0.5Bより高い。Llama互換の構造だが独自に学習したモデルで、Llamaのライセンスは関係しない |
| sarashina2.2-1b-instruct-v0.1 | SB Intuitions | 1B | MIT | 同上 | 同上。開発元の評価ではQwen2.5-1.5Bやllm-jp-3-1.8bと同等以上 |
| Qwen2.5-0.5B-Instruct / 1.5B-Instruct | Alibaba | 0.5B / 1.5B | Apache-2.0 | 公式あり | 多言語。日本語は日本語特化モデルより弱い |
| Qwen3-0.6B / 1.7B | Alibaba | 0.6B / 1.7B | Apache-2.0 | 公式あり | 思考モードを切って使う |
| llm-jp-3-1.8b-instruct3 | 国立情報学研究所 | 1.8B | Apache-2.0 | なし | 学習データの内訳が公開されている |
| RakutenAI-2.0-mini-instruct | 楽天 | 1.5B | Apache-2.0 | なし | 日英 |

- 採用するときは、版（Hugging Faceのコミット）とファイルのSHA-256を固定し、LICENSEをパッケージに入れる。確認したコミット: sarashina2.2-0.5b-instruct `e4b9aacc`、1b-instruct `08cf5a8a`、Qwen2.5-0.5B-Instruct-GGUF `9217f5db`（q4_k_m 491MB、SHA-256 `74a4da8c…`）
- モデルカードのライセンス表記とLICENSEファイルが食い違うもの、学習に別の規約（Gemmaなど）のデータやモデルを使ったものは除く
...
- 文脈変換・もしかして: 2B以下のモデルが必要。上の同梱候補をT-S06で比べて選ぶ。必要なら選んだモデルをかな漢字変換向けに追加学習する（学習に使うデータの権利も確認する）
...
- Windows: WiXインストーラー（TIP 3種の登録、サーバー、設定アプリ、辞書、同梱のSLM）、コード署名、アンインストール時の登録解除
- インストーラーの最後に設定アプリのウィザードを開き、次を選んで始められるようにする（R-08）。あとから設定アプリで変えられる
  - 日本語・英語の切替キー（左右Alt単押しなど）
  - 句読点と記号の入力（C-01〜C-03）
  - もしかして: オフ / 軽量（辞書とアルゴリズムだけ。SLMを読み込まない）/ 文脈あり（同梱のSLMを使う）。バッテリー駆動時に止めるか
  - 学習と予測の有効・無効
