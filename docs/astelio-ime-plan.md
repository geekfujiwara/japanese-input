...
実装済み: B-01〜B-02、B-04〜B-05、B-09〜B-10、B-13〜B-14、D-01、D-04、D-05（Windows TIP）。次の順で進める案。優先度は日常の使い勝手への影響で付けた。

| 優先 | ID | 内容 | 補足 |
| --- | --- | --- | --- |
| 高 | B-11 | パスワード欄・PIN欄で日本語入力にしない | 今は学習を止めるだけ。InputScopeで判定して直接入力にする |
| 高 | B-08 | 確定取り消し（確定直後のBackSpaceやCtrl+Backspaceで変換中に戻す） | 誤って確定したときの手戻りが大きい |
| 高 | B-06 | 一部確定（注目文節までを確定し、残りを変換中のまま残す） | 長い文の入力で使う |
| 高 | B-12 | 入力モードの切替時に、カーソルの近くへ「あ / A」を一時的に表示する | Alt単押しの切替が見えるようにする |
