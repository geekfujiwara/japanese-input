...
- 言語モデル: 品詞の連接コスト行列＋単語コスト。同音異義語は、前後の文節の意味（azooKeyの意味ID）の連接コストで選び直す（最良の区切りのまま、各文節の上位の候補から、隣の文節との意味のつながりが最もよい組み合わせを選ぶ）。後で単語の2-gramを追加する
...
| azooKey_dictionary_storage | システム辞書、連接コスト | Apache-2.0 | azooKeyのバイナリ形式（LOUDSのノードごとのエントリー、品詞の連接コスト行列、意味IDの連接（`mm.binary`））。辞書ツールでAstelioの6列TSV（読み・表記・左品詞ID・右品詞ID・意味ID・コスト）と連接コストのTSVに変換する。版はコミットとSHA-256で固定する。LICENSEの同梱が必要 |
