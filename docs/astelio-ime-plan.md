...
| D-04 | 自動学習（選んだ候補の順位、文節区切り、よく使う連続した語、予測候補）。選んだ候補は次から候補ウインドウの先頭に出し、履歴の印を付ける |
| D-05 | 学習履歴の閲覧、個別削除、期間を指定した削除、全削除。候補ウインドウでは Ctrl+Delete で選んだ候補を履歴から消す。タスクバーの入力モードのアイコンを右クリックして、履歴の使用の切替、管理画面（一覧と削除）、全削除ができる |
...
| 学習履歴 | テキスト（UTF-8、タブ区切り。最大5000件、古いものから消す） | `%LOCALAPPDATA%\AstelioIME\learning.tsv` | 同上フォルダ |
