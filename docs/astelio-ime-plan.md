...
実装済み: B-01〜B-02、B-04〜B-06、B-08〜B-14（B-07を除く）、D-01、D-04、D-05（Windows TIP）。次の順で進める案。優先度は日常の使い勝手への影響で付けた。

| 優先 | ID | 内容 | 補足 |
| --- | --- | --- | --- |
| 済 | B-11 | パスワード欄・PIN欄で日本語入力にしない | InputScopeで判定し、キーをそのままアプリに渡す |
| 済 | B-08 | 確定取り消し（確定直後のCtrl+BackSpaceで変換中に戻す） | 確定した文字列がカーソルの直前にあるときだけ消す。学習の記録は取り消さない |
| 済 | B-06 | 一部確定（Ctrl+↓で注目文節までを確定し、残りを変換中のまま残す） | |
| 済 | B-12 | 入力モードの切替時に、カーソルの近くへ「あ / A」を一時的に表示する | 外部からのオン・オフでは表示しない |
| 中 | B-07 | 再変換（確定済みの文字列を選んで変換し直す。TSFの `ITfFnReconversion`） | |
| 中 | D-02・D-03・D-08 | ユーザー辞書（追加・編集・インポート・エクスポート）と抑制語 | 管理画面は入力履歴の画面と同じ作りにできる |
| 中 | D-06 | シークレットモード（履歴を使いながら記録しない）とアプリごとの設定 | 右クリックメニューに加える |
