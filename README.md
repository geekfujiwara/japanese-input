...
| 💡 **もしかして** | `yu-a-` と打つと候補の2番目に `ユーザー`。隣のキー・キーの抜け・入れ替わり・母音の打ち間違いなどを辞書とアルゴリズムで直します（言語モデルは使いません） |
| 🕘 **入力の履歴** | 選んだ候補は次から先頭に出ます。候補ウインドウで `Ctrl`+`Delete` で消せます。タスクバーの「あ / A」を右クリックすると、履歴の切替・記録の一時停止（シークレットモード）・アプリごとの記録停止・管理・全削除ができます。パスワード欄では日本語入力にならず、記録もしません |
...
| `Tab` / `↓` | 予測候補を選ぶ（`えもじ` のときは絵文字パレットを開く） | 次の候補 |
| `←` `→` | カーソル移動 | 文節の移動 |
| `Shift` + `←` `→` | — | 文節の伸縮 |
| `1`〜`9`、`PageUp` / `PageDown` | — | 候補ウィンドウで選択、ページ送り |
| `Ctrl` + `Delete` | — | 候補ウィンドウで選んだ候補を入力の履歴から削除 |
| `Ctrl` + `↓` | — | 注目文節までを確定（残りは変換中のまま） |
| `Ctrl` + `BackSpace` | 確定の直後: 確定を取り消して変換中に戻す | — |
